...
class flash_gamelet : public gamelet
{
private:
    //game specific data
...
public:
	...
	{
		addidionalValues["Description"]="this is a sample gameled , Desgined to demostrated Game engine capacities\n it is designed to work with a flash client , even thoug other web conforming client should be fine too.";
		addidionalValues["Version"]="0.0.1 pre alpha";
		addidionalValues["clientPath"]="flash/api.js";
		addidionalValues["ClassID"]="dbd8f7e0-d525-11df-937b-0800200c9a66";
		addidionalValues["Name"]="flashSampleGamelet";
	}
...
/** @brief insers an object with description
  ...
  */
gamelet_object* flash_gamelet::add_object(point &cords,const string &obj_Type,const string &entity_id)
{
...
void flash_gamelet::add_object_auto(const std::string & game_session_id)
{
    point pt;
...
void flash_gamelet::add_object_auto_by_player(std::string & game_session_id,std::string & player_id)
{
...
void flash_gamelet::add_player(std::string & player_id)
{
...
void flash_gamelet::remove_player(std::string & player_id)
{
...
///will initiate
void flash_gamelet::init_gamelet(gamelet_desc *my_gamelet_desc)
{
	strcpy(this->gamelet_name,"flash_gamelet_ver_1_0_0");
    cout<<"init bounds\n";
    bounds[0][0]=0;//x1
    bounds[0][1]=0;//y1
    bounds[1][0]=1000;//x2
...
void flash_gamelet::get_model_string(std::string &msgout)
{
...
void flash_gamelet::handle_client_request(const handle_client_request_Args & req_Ags,std::string & reply)
{
    ...
    flash_gamelet_Utils::replaceAll(req,">"," ");
    flash_gamelet_Utils::replaceAll(req,";"," ");
	//unescape by replacing '>', ';' with ' ' (whitespace)
    //read values
...
extern "C" gamelet* make()
{
    ...
    return new flash_gamelet();
}


void flash_gamelet::get_name(char *name)
{
	strcpy(name,"flashSampleGamelet");
}

void flash_gamelet::get_path(char *path)
{
	strcpy(path,"flash/api.js");
}